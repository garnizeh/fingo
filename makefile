...
KIND_CLUSTER    := garnizeh-starter-cluster
NAMESPACE       := fingo-system
FINGO_APP       := fingo
AUTH_APP        := auth
BASE_IMAGE_NAME := localhost/garnizeh
VERSION         := 0.0.1
FINGO_IMAGE     := $(BASE_IMAGE_NAME)/$(FINGO_APP):$(VERSION)
...
build: fingo metrics auth

fingo:
	docker build \
		-f zarf/docker/dockerfile.fingo \
		-t $(FINGO_IMAGE) \
...
dev-load:
	kind load docker-image $(FINGO_IMAGE) --name $(KIND_CLUSTER) & \
...
dev-apply:
	...
	kustomize build zarf/k8s/dev/fingo | kubectl apply -f -
	kubectl wait pods --namespace=$(NAMESPACE) --selector app=$(FINGO_APP) --timeout=120s --for=condition=Ready

dev-restart:
	kubectl rollout restart deployment $(AUTH_APP) --namespace=$(NAMESPACE)
	kubectl rollout restart deployment $(FINGO_APP) --namespace=$(NAMESPACE)
...
dev-logs:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(FINGO_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | go run api/tooling/logfmt/main.go -service=$(FINGO_APP)
...
dev-logs-init:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(FINGO_APP) -f --tail=100 -c init-migrate-seed

dev-describe-node:
	kubectl describe node

dev-describe-deployment:
	kubectl describe deployment --namespace=$(NAMESPACE) $(FINGO_APP)

dev-describe-fingo:
	kubectl describe pod --namespace=$(NAMESPACE) -l app=$(FINGO_APP)
...
dev-services-delete:
	kustomize build zarf/k8s/dev/fingo | kubectl delete -f -
...
dev-describe-replicaset:
	kubectl get rs
	kubectl describe rs --namespace=$(NAMESPACE) -l app=$(FINGO_APP)
...
dev-shell:
	kubectl exec --namespace=$(NAMESPACE) -it $(shell kubectl get pods --namespace=$(NAMESPACE) | grep fingo | cut -c1-26) --container $(FINGO_APP) -- /bin/sh
...
migrate:
	export FINGO_DB_HOST=localhost; go run api/tooling/admin/main.go migrate

seed: migrate
	export FINGO_DB_HOST=localhost; go run api/tooling/admin/main.go seed
...
token-gen:
	export FINGO_DB_HOST=localhost; go run api/tooling/admin/main.go gentoken 5cf37266-3473-4006-984f-9325122678b7 54bb2165-71e1-41a6-af3e-7da4a0e1e2c1
...
run:
	go run api/services/fingo/main.go | go run api/tooling/logfmt/main.go

run-help:
	go run api/services/fingo/main.go --help | go run api/tooling/logfmt/main.go
...
curl-create:
	...
	-d '{"name":"dev","email":"d@gmail.com","roles":["ADMIN"],"department":"ITO","password":"123","passwordConfirm":"123"}' \
...
talk-logs:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(FINGO_APP) --all-containers=true -f --tail=100 --max-log-requests=6

talk-logs-cpu:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(FINGO_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | grep SCHED

talk-logs-mem:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(FINGO_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | grep "ms clock"

talk-describe:
	kubectl describe pod --namespace=$(NAMESPACE) -l app=$(FINGO_APP)

talk-describe-min:
	kubectl describe pod --namespace=$(NAMESPACE) -l app=$(FINGO_APP) | grep -i -A 21 '  fingo:' | sed -n '1p;3p;11,16p;18,22p'
...
help:
	...
	@echo "  fingo                   Build the fingo container"
	...
	@echo "  dev-logs                Show the logs for the fingo service"
	...
	@echo "  dev-describe-fingo      Show the fingo pod details"
...
tilt-logs:
	@echo "Usage: make tilt-logs SERVICE=fingo"
	@if [ -z "$(SERVICE)" ]; then \
		echo "Error: SERVICE required"; \
		echo "Example: make tilt-logs SERVICE=fingo"; \
		exit 1; \
	fi
	tilt logs $(SERVICE)
...
# Usage: make chart-install CHART=fingo ENV=prod
chart-install:
	@if [ -z "$(CHART)" ]; then \
		echo "Error: CHART required"; \
		echo "Example: make chart-install CHART=fingo ENV=prod"; \
		exit 1; \
	fi
...
chart-upgrade:
	@if [ -z "$(CHART)" ]; then \
		echo "Error: CHART required"; \
		echo "Example: make chart-upgrade CHART=fingo ENV=prod"; \
		exit 1; \
	fi
...
chart-uninstall:
	@if [ -z "$(CHART)" ]; then \
		echo "Error: CHART required"; \
		echo "Example: make chart-uninstall CHART=fingo"; \
		exit 1; \
	fi
...
chart-template:
	@if [ -z "$(CHART)" ]; then \
		echo "Error: CHART required"; \
		echo "Example: make chart-template CHART=fingo ENV=dev"; \
		exit 1; \
	fi
...
chart-lint:
	@if [ -z "$(CHART)" ]; then \
		echo "Error: CHART required"; \
		echo "Example: make chart-lint CHART=fingo"; \
		exit 1; \
	fi
